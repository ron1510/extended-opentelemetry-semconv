...
/** Serializes the event JSON from the second field of a two-column sink row. */
public final class SecondColumnStringSerializationSchema implements SerializationSchema<Row> {
    private static final long serialVersionUID = 1L;

    @Override
    public byte[] serialize(final Row element) {
        return requiredString(element, 1).getBytes(StandardCharsets.UTF_8);
    }
