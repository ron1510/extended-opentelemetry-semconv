...
import extended_otel_semconv_codegen.generator as codegen
from extended_otel_semconv_codegen.generator import (
    GenerationPaths,
    _class_name,
    _domain_name,
    _field_name,
...
PACKAGE_SOURCE = (
    REPOSITORY_ROOT / "packages" / "extended-opentelemetry-semconv-models" / "src" / "extended_otel_semconv"
)
...
def test_renderers_cover_empty_single_and_multiple_domains() -> None:
    ...
    package = _render_package_init((generated,), (relationship,), ["app"])
    ...
    assert '"app.endpoint": AppEndpoint' in package
